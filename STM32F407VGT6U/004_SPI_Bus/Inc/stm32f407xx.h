...
#define 	SPI1_BASE			0x40013000U  		// APB2
#define 	SPI2_BASE			0x40003800U 		// APB1
#define 	SPI3_BASE			0x40003C00U 		// APB1


// Base addresses linked to belonging to theres own register definitions
...
#define		GPIOA				((GPIO_RegDef_t*) GPIOA_BASE)		// 8.4 GPIO registers
...
#define 	SPI1				((SPI_RegDef_t*) SPI1_BASE)		// 28.5 SPI and I2S registers
...
{									// Address offset
	...
	__vo	uint32_t AFR[2];		// 0x20		8.4.9 GPIO alternate function low/high register (GPIOx_AFRL, GPIOx_AFRH)
...
typedef struct 						// 28.5 SPI and I2S registers
{									// Address offset
	__vo	uint32_t SPI_CR1;		// 0x00	 	28.5.1 SPI control register 1 (SPI_CR1) (not used in I2S mode)
	__vo	uint32_t SPI_CR2;		// 0x04 	28.5.2 SPI control register 2 (SPI_CR2)
	__vo	uint32_t SPI_SR;		// 0x08 	28.5.3 SPI status register (SPI_SR)
	__vo	uint32_t SPI_DR;		// 0x0C 	28.5.4 SPI data register (SPI_DR)
	__vo	uint32_t SPI_CRCPR;		// 0x10 	28.5.5 SPI CRC polynomial register (SPI_CRCPR) (not used in I2S mode)
	__vo	uint32_t SPI_RXCRCR;	// 0x14 	28.5.6 SPI RX CRC register (SPI_RXCRCR) (not used in I2S mode)
	__vo	uint32_t SPI_TXCRCR;	// 0x18 	28.5.7 SPI TX CRC register (SPI_TXCRCR) (not used in I2S mode)
	__vo	uint32_t SPI_I2SCFGR;	// 0x1C 	28.5.8 SPI_I2S configuration register (SPI_I2SCFGR)
	__vo	uint32_t SPI_I2SPR;		// 0x20 	28.5.9 SPI_I2S prescaler register (SPI_I2SPR)
